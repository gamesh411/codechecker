...
class TestCtu(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # so the tests work.
        def adjust_buildlog(buildlog_filename: str):
            # Read the template from test directory.
            original_buildlog_path = os.path.join(
                self.test_dir, buildlog_filename)
            with open(original_buildlog_path,
                      encoding="utf-8", errors="ignore") as log_file:
                build_json = json.load(log_file)

            # Fill the template.
            for command in build_json:
                command['directory'] = str(self.test_dir)

            # Write it to the workspace directory.
            workspace_buildlog = os.path.join(
                self.test_workspace, buildlog_filename)
            with open(workspace_buildlog, 'w',
                      encoding="utf-8", errors="ignore") as log_file:
                json.dump(build_json, log_file)

        adjust_buildlog('buildlog.json')
        adjust_buildlog('complex_buildlog.json')
    ...
    def test_ctu_ondemand_yaml_format(self):
        ...
        cmd = [self._codechecker_cmd, 'analyze',
               ...
               '--ctu-collect',  # ctu-directory is needed, and it remains
                                 # intactonly if a single ctu-phase is
                                 # specified
